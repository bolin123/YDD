...
static void protocolSendDataHead(SysDataInfo_t *info)
{
    ...
    if(info != NULL)
    {
        ...
        dataHead.pid = info->id;
        dataHead.date = info->date;
    ...
    if(g_dataTransferStart)
    {
        ...
        else
        {
            if(SysTimeHasPast(currentTranfer->lastTransTime, 100))
            {
                ...
                {
                    SysDataGetRaw(currentTranfer->address + currentTranfer->size, buff, PROTOCOL_DATA_PACKET_LENTH);
                ...
                else
                {
                    ...
                    if(lastDatalen)
                    {
                        SysDataGetRaw(currentTranfer->address + currentTranfer->size, buff, lastDatalen);
...
static void protocolUartInit(void)
{
    ...
    HalUartConfig(HAL_UART_PORT_1, &config);
}
